...
public class PublicPagoController {
    ...
    public ResponseEntity<PagoPublicPreferenceResponseDto> crearPreferencia(
            @PathVariable String codigoReserva,
            @RequestHeader("X-Reservation-Token") String accessToken) {
        return ResponseEntity.ok(pagoService.crearOReutilizarPreferencia(codigoReserva, accessToken));
    }
    ...
    public ResponseEntity<PagoReturnResponseDto> reconciliarRetorno(
        ...
        return ResponseEntity.ok(pagoService.reconciliarRetorno(
                codigoReserva, accessToken, request.getPaymentId()));
    }
}
